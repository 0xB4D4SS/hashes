...
class Hash:
    ...
    def generateHash(self, data):
        res = self.encryptBlock(self.key)
        run = True
        resprev = res
        while run:
            temp = data[:self.w4]
            hasher = Hash(64, 20, temp)
            if len(temp) != self.w4:
                data = data.ljust(self.w4, b'\x00')
                run = False
            rescurr = int.from_bytes(hasher.encryptBlock(resprev), byteorder='little') ^ int.from_bytes(resprev, byteorder='little')
            res = rescurr.to_bytes(rescurr.bit_length(), byteorder='little')
            data = data[self.w4:]
            if not data:
                break
        return res
...
if __name__ == '__main__':
    ''' We take rc5 cipher to generate hash block-by-block. block size is set to 64. '''
    # Let's say our hash formula is H[i] = E[M[i]](H[i-1]) xor H[i-1]
    w = 64
    data = bytes(input("Input text: "), 'utf-8')
    hashgen = Hash(w, 20, startkey)
    resstr = int.from_bytes(bytes=hashgen.generateHash(data), byteorder='little')
    print(hex(resstr))
    print(len(hex(resstr)))
    # PLS DONT WRITE 'test'